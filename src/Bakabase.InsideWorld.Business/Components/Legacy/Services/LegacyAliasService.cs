...
namespace Bakabase.InsideWorld.Business.Components.Legacy.Services
{
        ...
        : FullMemoryCacheResourceService<InsideWorldDbContext, LegacyAlias, int>(serviceProvider)
    {
        ...
        // /// any - alias group
        // /// </returns>
        // public async Task<Dictionary<string, HashSet<string>>> GetFullMap()
        // {
        //     var aliases = (await GetAll(null, false))!;
        //     var map = aliases
        //         .GroupBy(x => x.GroupId).Select(x => x.Select(y => y.Name).ToHashSet())
        //         .SelectMany(x => x.Select(y => (Any: y, Set: x))).ToDictionary(x => x.Any, x => x.Set);
        //     return map;
        // }
    }
}